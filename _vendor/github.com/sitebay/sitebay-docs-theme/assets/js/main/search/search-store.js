...
export function newSearchStore(searchConfig, Alpine) {
	...
	let store = {
		...
		withBlank: async function (callback = () => {}) {
			...
			const response = await fetch('/support/data/sections/index.json');
			if (response.ok) {
				...
				newRequestCallback(
					{
						...
						// as the tutorials is the most populated section tree.
						filters:
							'NOT section:tutorials AND NOT section:api AND NOT section:features AND NOT section:content AND NOT section:development',
					},
					(result) => {
...
const normalizeResult = function (self, result) {
	...
	result.sections = function () {
		...
		for (let i = 0; ; i++) {
			...
			for (let k in sectionFacets) {
				...
				let hasObjectID = sectionLvl0 == 'features' || sectionLvl0 == 'tutorials';
				position++;
	...
	result.hits.forEach((hit, idx) => {
		...
		if (!hit.thumbnailUrl) {
			hit.thumbnailUrl = '/support/media/images/Site Bay-Default-416x234.jpg';
		}
